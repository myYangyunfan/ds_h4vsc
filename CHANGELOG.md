...
2. **修复**：改为**等选区停止变动 0.6 秒后再显示**，而选区一变就立即清除。于是拖动过程中完全不出现，松手（或停止按方向键）后才出现一次；切换编辑器、收起选区、关闭该设置都会立即清掉。另外清除时遍历全部可见编辑器，避免在已不活跃的编辑器上残留
3. 设置描述同步说明了这个等待行为
...
2. **根因**：模型与推理档位是**会话级**状态，内核只在 `session/new` 或 `session/resume` 的响应里下发。仅恢复本地记录的面板自然拿不到，`configOptions` 为空；而 0.9.2 加的"空工具条不渲染"守卫把整行隐藏，于是看起来像功能不存在
3. **修复**：新增 `primeSession()`，在恢复时间线后主动把该会话在内核中打开（必要时先恢复），从而拿到它的配置项；失败只记日志，不影响用户随后发送消息。`submitPrompt` 原有路径不受影响
4. **可诊断性**：准备完成后记录 `Prepared session <id> with N config option(s)`。此前"控件不见"在日志上与"内核没被访问"毫无区别，这是本次排查绕远的原因

## 0.9.2 — 模型选择与上下文用量真正看得见

1. **问题**：0.9.0 把模型/推理档位下拉与上下文用量加在了面板头部，但用户找不到。根因是布局——`.header-actions` **不换行**（只有外层 `.header` 会），而聊天面板位于右侧窄栏（约 320–380px），一行根本放不下标题、两个下拉、余额、用量、历史下拉和三个图标按钮，新控件被挤出可见区域
2. **重排头部为两行**：第一行放标题与图标按钮；第二行是控件条（模型、推理档位、余额、上下文用量、历史会话），`flex-wrap: wrap` 自动折行。实测 360px 与 320px 下横向溢出均为 0，没有被裁切
3. **控件可辨识**：每个下拉前加图标（🤖 模型 / 💡 推理档位 / 💳 余额 / 🥧 用量），并有 `title` 与 `aria-label`——此前只有提示文本，露出来也不容易认出是哪个
4. **空工具条不占位**：内核不提供配置项时（或没有用量、余额、历史可显示），整行不渲染，避免窄栏里白占一行高度
5. **验证方式**：用真实内核返回的数据（分组模型选项、`reasoning_effort` 四档、`usage_update`、余额响应）在浏览器里按真实侧栏宽度渲染截图确认，而非仅凭类型检查

## 0.9.1 — 会话不再"看起来没保存"

1. **根因（读实际持久化数据得出，非推断）**：`SessionStore.upsert` **只在面板自己新建会话时调用**。而用户那次是从左侧列表**打开**内核里既有的会话（`loadSession`），那条路径从不登记。于是出现了这样的状态：`dsh.timelines` 里存着一段 6 条消息的对话，而 `dsh.sessions` **是空的**——对话内容在，历史列表里却一条都没有，于是"新对话没保存"
2. **修复**：新增 `ensureSessionListed()`，凡是有内容的会话都登记进历史列表，无论它是面板新建的、重载后恢复的、还是从内核历史里接管的；已有条目不动，用户重命名或 AI 生成的标题不会被覆盖
3. **启动恢复时一并登记**：从存储恢复的那段对话此前也不在列表里，现在会登记，标题取该对话的第一条用户消息，因此重载后立刻可见
4. **可诊断性**：每次落盘记一条 DEBUG 日志（`Persisted N entries for <id>`）。这次排查本可以少绕一圈——日志里既没有落盘记录、也没有失败记录，我一度只能靠外部推断
5. **顺带修掉一处不一致**：旧单槽位数据迁移时，那条记录原先每次读取都被打上"当前时间"，会永久占据"最近会话"。改为恒定最旧值（`save` 会把迁移结果落盘，所以影响窗口很窄；已加跨重载的用例记录该优先级）。**需要说明：变异验证显示这条改动并非用户所遇问题的原因**，真正原因是第 1 条
...
2. **按会话分别持久化**：新增 `TimelineStore`（不依赖 vscode，可单测），以 sessionId 为键保存每个会话的对话与工作集，保留最近 20 个；打开会话时恢复其内容；删除会话时一并清除；切换/新建对话不再销毁上一个
3. **兼容旧布局**：读取时会把老的单槽位数据并入，升级不丢当前屏幕上的那段对话
4. **空白不再是无声的**：若某会话确实没有保存内容（本版本之前创建的会话），打开时明确告知，而不是让人对着空面板猜
5. **测试**：`TimelineStore` 新增 9 项用例，覆盖"写 s2 不摧毁 s1"、工作集往返、最近会话判定、上限淘汰、旧布局迁移、删除清理。写测试时还发现并修掉一个真实弱点：同一毫秒内保存两个会话时 `Date.now()` 并列，"最近"变得不确定——现在写入的时间戳严格递增
