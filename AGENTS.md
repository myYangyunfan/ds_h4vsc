...
- **Kernel discovery order** (`DshLocator`): `dsh.executablePath` setting → managed install in `globalStorage/dsh` (npm `--prefix`, run with `ELECTRON_RUN_AS_NODE=1` so no system Node is needed) → `dsh` on the inherited PATH, then off-PATH. **A GUI-launched editor does not inherit the terminal's PATH** — on macOS it gets launchd's minimal `/usr/bin:/bin:/usr/sbin:/sbin`, so a kernel installed by Homebrew, nvm/fnm/Volta/Bun or npm's prefix is invisible to a bare `execFile('dsh')`. `kernelPaths.ts` (pure, unit tested) lists the well-known locations and the version-manager globs, and `DshLocator` falls back to the login shell's own `$PATH` (`$SHELL -ilc`, marker line, cached, 5s timeout). `npm` is resolved the same way, because "Install kernel" fails with ENOENT on those machines too. Do not "simplify" this back to a single `execFile`.
